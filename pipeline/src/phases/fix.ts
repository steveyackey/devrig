...
export async function fix(config: PipelineConfig, milestoneIndex: number): Promise<PhaseResult> {
  ...
  const verificationStatusJson = await readMilestone(
    config.workDir,
    milestoneIndex,
    "verification-status.json",
  );
  const verificationStatus = JSON.parse(verificationStatusJson);
  const failedChecks: VerificationCheck[] = verificationStatus.checks.filter(
    (c: VerificationCheck) => !c.passed,
  );
  ...
  if (await existsMilestone(config.workDir, milestoneIndex, "verification-results.md")) {
    verificationResults = await readMilestone(
      config.workDir,
      milestoneIndex,
