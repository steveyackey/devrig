...
export async function read(workDir: string, filename: string): Promise<string> {
  return readFile(join(workDir, filename), "utf8");
}
...
export async function existsMilestone(
  workDir: string,
  milestoneIndex: number,
  filename: string,
): Promise<boolean> {
  return access(join(milestoneDir(workDir, milestoneIndex), filename)).then(
    () => true,
    () => false,
  );
}
