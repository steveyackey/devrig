...
export async function runPipeline(config: PipelineConfig): Promise<void> {
  ...
  for (let i = startFrom; i < milestoneCount; i++) {
    ...
    for (let attempt = 1; attempt <= config.maxRetries; attempt++) {
      ...
      if (attempt === 1) {
        ...
        await notify(
          `devrig-pipeline: ${milestone.version} — running targeted fix (attempt ${attempt})`,
        );
    ...
    if (passed) {
      ...
      const result = await new Promise<{ exitCode: number; stderr: string }>((resolvePromise) => {
        execFile("git", ["-C", config.repoRoot, "add", "-A"], () => {
          execFile(
            "git",
            ["-C", config.repoRoot, "commit", "-m", commitMsg],
            (commitErr, _o, commitStderr) => {
              if (commitErr) {
                resolvePromise({ exitCode: 1, stderr: commitStderr });
                return;
              }
              execFile("git", ["-C", config.repoRoot, "push"], (pushErr, _po, pushStderr) => {
                resolvePromise({ exitCode: pushErr ? 1 : 0, stderr: pushStderr });
              });
            },
          );
        });
      ...
      if (result.exitCode === 0) {
        ...
        await notify(
          `devrig-pipeline: ⚠ ${milestone.version} git commit/push failed (exit ${result.exitCode})`,
        );
      }
    }
  ...
  await notify(
    `devrig-pipeline: COMPLETE — all milestones done ($${state.total_cost.toFixed(2)} total)`,
  );
